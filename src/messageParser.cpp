...
void xmlServer::MessageParser::register_response_callback(const uint32_t id, response_callback callback)
{
    if(callback)
        response_callbacks_[id] = callback;
}

void xmlServer::MessageParser::register_notification_callback(const std::string &notification, jsonrpcpp::notification_callback callback)
{
    if(callback)
        notification_callbacks_[notification] = callback;
}

void xmlServer::MessageParser::register_request_callback(const std::string &request, jsonrpcpp::request_callback callback)
{
    if(callback)
        request_callbacks_[request] = callback;
}

jsonrpcpp::entity_ptr xmlServer::MessageParser::parse(const std::string &json_str)
{
